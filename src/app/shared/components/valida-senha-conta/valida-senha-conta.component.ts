...
import { TipoValidacaoSenha, Telefone, ValidacaoToken, ValidacaoSenha, TokenValidacaoRequest, TipoValidacaoSelecionada } from "./valida-senha-conta.model";
...
@Component({
  ...
  selector: "cx-valida-senha-conta",
  templateUrl: "./valida-senha-conta.component.html",
  styleUrls: ["./valida-senha-conta.component.css"]
})
export class ValidaSenhaContaComponent implements OnInit, OnChanges {
 ...
 */
  ...
  */
  ...
  /**
   * Controla a exibição do modal de recadastramento de senha
  */
  ...
  private buscaContas(): void {
    if (objIsNotEmpty(this.comercialService.clienteAtual)) {
      ...
      if (this.cliente.tipoCliente === TipoCliente.PF) {
        params["cpf"] = this.cliente.resumoCliente.dadosBasicosPessoaFisicaDTO.cpf;
      } else {
        params["cnpj"] = this.cliente.resumoCliente.dadosBasicosPessoaJuridicaDTO.cnpj;
      }
  ...
  */
  public consultaListaTelefonesAdesaoSms(): void {
    ...
    if (this.cliente.tipoCliente === TipoCliente.PF) {
      tipoContaToken = this.cliente.resumoCliente.dadosBasicosPessoaFisicaDTO.cpf;
    } else {
      tipoContaToken = this.cliente.resumoCliente.dadosBasicosPessoaJuridicaDTO.cnpj;
    }
  ...
  public enviarToken(): void {
    ...
    const tokenCriacaoRequest = {
      "documento": this.cliente.tipoCliente === 1 ? this.cliente.resumoCliente.dadosBasicosPessoaJuridicaDTO.cnpj : this.cliente.resumoCliente.dadosBasicosPessoaFisicaDTO.cpf,
  ...
  public abrirModalCadastroSenha() {
    this.passwordControl.setValue("");
    this.contaSelecionada = this.convertDadosContaToConta(this.dadosConta);

    if (this.contaSelecionada && this.contaSelecionada.icConta === "1" && this.contaSelecionada.titularidade !== 1) {
      throw new Error("Para essa Operação de Conta, somente o primeiro titular pode alterar a senha");
    } else {
      this.cliente.senhaConta.showFormulario = true;
      this.cliente.senhaConta.controleContas.contaAtual = this.buscaContasService.getContaFormatada(this.contaSelecionada);
      this.comercialService.atualizaClienteAtual(this.cliente);
  ...
  private gerarCriptograma(): void {
    ...
    this.consultaCorporativaService.gerarCriptograma(senha, dadosConta).subscribe(senhaCript => {
      const validarSenhaRequest: ValidaSenhaContaRequest = {
        ...
        titularidade: dadosConta.titular,
        cpfCnpj: this.cliente.tipoCliente === 1 ? this.cliente.resumoCliente.dadosBasicosPessoaJuridicaDTO.cnpj : this.cliente.resumoCliente.dadosBasicosPessoaFisicaDTO.cpf,
